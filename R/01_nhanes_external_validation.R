...
#         calibration slope from logistic recalibration,
#         calibration-in-the-large (linear predictor as fixed offset),
#         decile calibration, 0.5-threshold classification metrics).
#
...
# The paper reports calibration as slope 0.40 (0.36-0.44) and
# calibration-in-the-large -1.37 (-1.48 to -1.27) for this cohort.
#
...
cat(sprintf("PR-AUC = %.4f (stepwise average precision; platform estimator 0.5175)\n", prauc))
...
lp <- log(pe / (1 - pe))
calmod  <- glm(y ~ lp, family = binomial())            # slope + 2-parameter intercept
calmod2 <- glm(y ~ 1, offset = lp, family = binomial()) # calibration-in-the-large
cat(sprintf("Calibration: slope = %.4f | recalibration intercept (alpha) = %.4f | calibration-in-the-large = %.4f\n",
            unname(coef(calmod)[2]), unname(coef(calmod)[1]), unname(coef(calmod2)[1])))
...
# Paper values at reported precision (calibration-in-the-large printed as -1.37, 2 dp).
expected <- c(AUC = 0.7357, Brier = 0.2271, slope = 0.4002, citl = -1.3746,
              sens = 0.6873, spec = 0.6714, ppv = 0.4241, npv = 0.8590, acc = 0.6755)
got <- c(AUC = auc, Brier = brier, slope = unname(coef(calmod)[2]),
         citl = unname(coef(calmod2)[1]),
...
cat(sprintf("\n%d/%d reported metrics agree with the paper.\n", sum(chk$match), nrow(chk)))
cat("(calibration-in-the-large is reported as -1.37 in the paper; the 2-parameter\nrecalibration intercept is printed above for completeness only.)\n")
